...
function importance_sampling(log_target, std_normal_draws) 
    particles = std_normal_draws
    function log_weight(particle) 
        # this function, gets compiled to GPU, for example trying to allocate (e.g., 'x = ones(1)') will crash
        return log_target(particle) - (-particle^2 / 2) # proposal is std normal (this function will be compiled to GPU!)
    end
    log_weights = log_weight.(particles) 
    log_norm = exp_normalize!(log_weights)
    return (; log_norm, particles, weights = log_weights)
end

function integrate(is_output, test_fct = Base.identity) 
    test_fct_evals = test_fct.(is_output.particles) 
    return sum(test_fct_evals .* is_output.weights)
end
